...
def Flatten_retriever_tool_eval(question_data, tool_path, tool_embedding_path, model_name):
    ...
    for d in tool_data:
        if "tool_code" in d:
            if "def " in d["tool_code"]:
                function_name = d["tool_code"].split("def ")[1].split("(")[0].strip()
                d["tool_code"] = d["tool_code"].replace(function_name, "execute")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_data_path", type=str, default="/Users/murong.yue/Desktop/data/superGPQA_test_data_math.json")
