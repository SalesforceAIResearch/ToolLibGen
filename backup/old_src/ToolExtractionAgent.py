...
from utils import LLMCaller, validate_tools_syntax
...
class ToolExtractionAgent:
        ...
        for i, (tool_num, content) in enumerate(matches):
            try:
               ...
               #  schema_content = content.split("</tag>")[1].split("<code>")[0]
                ...
                # Clean up tool_code - remove markdown formatting
                if "```python\n" in tool_code:
    ...
    # def agentic_tool_generation(self, question: str, answer: str) -> str:
    #     """Legacy method that returns raw LLM response."""
    #     prompt = TOOL_EXTRACT_AGENT_AGNTIC_GENERATION_PROMPT.format(question=question, answer=answer)
    #     response = self.llm.call(content=prompt)
    #     return response
