...
from utils import read_json, call_openai_api, call_openai_api_multi_turn, map_with_progress, validate_code_syntax, execute_code, apply_patch
from prompt import BLUEPRINT_DESIGN_PROMPT, CODE_IMPLEMENTATION_PROMPT, CODE_INSPECTOR_PROMPT_REVISE, LIB_REFINEMENT_BLUEPRINT_PROMPT, OPENAI_TOOL_IMPLEMENTATION_PROMPT
...
class ToolAggregationAgent:
    ...
    def _design_blueprint(self, cluster_name: str, tools: List[Dict]) -> Tuple[bool, List[Dict], Optional[str]]:
        """
...
Corrected JSON:"""
    ...
    def _quick_validate_tool_execution(self, tool_code: str, timeout: int = 3) -> Dict[str, Any]:
        """Quick validation by directly executing tool code - no LLM needed"""
        try:
...
""", temp_module)
            ...
            exec(tool_code, temp_module)
            
            # Check for execute function
            if 'execute' not in temp_module:
                return {
                    "is_valid": False,
            ...
            if not callable(execute_func):
                return {
                    "is_valid": False, 
                    "error": "'execute' is not callable",
                    "can_call": False
                }
            
            # Check for duplicate functions
            execute_count = tool_code.count('def execute(')
            if execute_count > 1:
                return {
                    "is_valid": False,
                    "error": f"Multiple 'execute' function definitions found ({execute_count})",
                    "can_call": False
                }
...
Please provide another fix in the same unified diff format. Learn from the previous attempts and avoid repeating the same mistakes."""
    ...
    def _optimize_sib_tool(self, cluster_name: str, validated_tool: Dict, sib_questions: List[Dict], sib_index: int, max_rounds: int = 3) -> Dict:
    ...
    def _optimize_single_question_for_sib(self, task_data: Tuple) -> Dict:
        ...
        try:
            ...
            # Run optimization
            result = optimizer.optimize_library_with_running_weaker_llm()
...
"""
    ...
    def _generate_tool_json_with_retry(self, messages: List[Dict], step_name: str, max_retries: int = 3, 
        ...
        2) Generate tool_info via OPENAI_TOOL_IMPLEMENTATION_PROMPT using the produced code
