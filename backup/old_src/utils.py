...
dotenv.load_dotenv()
sfr_api_key = os.getenv('SALESFORCE_SFR_API_KEY')
...
# Deprecated functions - use call_llm() instead for new code

# DEPRECATED: Use call_llm(model_name, content=..., provider="gateway") instead
def call_deepseek_v3_api(
    content: str,
    api_key: str = None,
    stream: bool = False,
    model_url: str = "https://gateway.salesforceresearch.ai/deepseek-v3-0324/process"
) -> dict:
    """
    Call the DeepSeek V3 API with the given content and return the response as a dict.

    Args:
        content (str): The message content to send to the model.
        api_key (str): The API key for authentication. If None, will use SALESFORCE_SFR_API_KEY from env.
        stream (bool): Whether to stream the response. Defaults to False.
        model_url (str): The API endpoint URL. Defaults to DeepSeek V3 endpoint.

    Returns:
        dict: The API response as a dictionary.
    """
    if api_key is None:
        api_key = os.getenv('SALESFORCE_SFR_API_KEY')
    headers = {
    ...
    try:
        ...
        print(f"Error calling DeepSeek V3 API: {e}")
        return None


# DEPRECATED: Use call_llm(model_name, messages=..., provider="gateway") instead
def call_deepseek_r1_32b_api(
    messages: list,
    model_url: str = "https://gateway.salesforceresearch.ai/deepseek-r1-32b/process"
) -> dict:
    """
    Call the DeepSeek-R1-Distill-Qwen-32B API with the given content and return the response as a dict.

    Args:
        messages (list): The message content to send to the model.
        api_key (str): The API key for authentication. If None, will use SALESFORCE_SFR_API_KEY from env.
        model_url (str): The API endpoint URL. Defaults to DeepSeek-R1-32B endpoint.
    ...
    try:
        ...
        print(f"Error calling DeepSeek-R1-32B API: {e}")
        return None


# DEPRECATED: Use call_llm(model_name, content=..., provider="openai") instead 
#  
# def call_openai_api(content: str, model_name: str, stream: bool = False, stop_tokens_lst: list = None):
#     try:
#         response = openai_client.chat.completions.create(
#             model=model_name,
#             messages=[{"role": "user", "content": content}],
#             stream=stream,
#             stop=stop_tokens_lst
#         )
...
# DEPRECATED: Use call_llm(model_name, messages=..., provider="gateway") instead
def call_llama3_70b_api(
    messages: list,
    model_url: str = "https://gateway.salesforceresearch.ai/llama3-70b/chat/completions"
) -> dict:
    """
    Call the DeepSeek-R1-Distill-Qwen-32B API with the given content and return the response as a dict.
    ...
    try:
        ...
        print(f"Error calling llama3-70b API: {e}")
        return None
...
def call_sfr_embedding_api(
    ...
    model_url: str = "https://gateway.salesforceresearch.ai/sfr-embedding/process"
) -> list:
    """
...
#         else:
#             print(f"Warning: No 'embeddings' key found in response. Available keys: {list(response_json.keys())}")
#             return None
            
#     except requests.exceptions.RequestException as e:
#         print(f"Error calling SFR Embedding API (HTTP): {e}")
#         return None
#     except json.JSONDecodeError as e:
#         print(f"Error parsing JSON response: {e}")
#         return None
#     except Exception as e:
#         print(f"Error calling SFR Embedding API: {e}")
#         return None
