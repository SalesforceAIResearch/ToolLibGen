...
"""
    ...
    def _extract_sig_and_doc_from_code(self, code: str) -> Tuple[str, str]:
        """
        从代码中提取 execute 函数的签名和 docstring。
        返回 (signature_line, docstring_text)，都带适当格式。
        """
        try:
            # 尝试解析 AST
            tree = ast.parse(code)
        except SyntaxError:
            # 如果语法错误，fallback 到简单字符串处理
            ...
            if isinstance(node, ast.FunctionDef) and node.name == "execute":
                try:
                    ...
                    if hasattr(ast, 'unparse'):  # Python 3.9+
                        ...
                        # Python 3.8 fallback - 使用 astor 或简单处理
    ...
    def _fallback_extract_sig_and_doc(self, code: str) -> Tuple[str, str]:
        ...
        for line in lines:
            ...
            if not in_docstring and sig_lines and (stripped.startswith('"""') or stripped.startswith("'''")):
                ...
                if stripped.count(docstring_quote) >= 2:  # 单行 docstring
                    in_docstring = False
                continue
...
#                '''
    ...
    def _optimize_sib_text_only(self, cluster_name: str, validated_tool: Dict, sib_tools: List[Dict], sib_index: int) -> Dict:
        try:
            original_sib_text = ''
            if isinstance(validated_tool, dict):
                original_sib_text = validated_tool.get('sib_text', '') or ''
            tools_for_eval = sib_tools[:10] if isinstance(sib_tools, list) else []
    ...
    def _process_single_sib_complete(self, args: Tuple) -> Tuple[bool, Optional[Dict], Dict]:
        try:
            ...
            if sib_tools_for_eval:
                ...
                optimized_text_info = self._optimize_sib_text_only(cluster_name, { 'sib_text': sib.get('content', '') }, sib_tools_for_eval, sib_index)
    ...
    def process_single_sib(self, cluster_name: str, tools: List[Dict], sib: Dict, output_dir: Path) -> Tuple[bool, Optional[Dict], Dict[str, Any]]:
