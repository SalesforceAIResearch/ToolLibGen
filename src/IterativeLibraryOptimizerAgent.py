...
"""
    ...
    def optimize_library_with_running_weaker_llm_using_sib(self, SIB: str = ""):
        """
...
Please answer the question using the provided tools. Think step by step about how to use the tools to solve the problem.
You should try to build the relationship between the tools and the question.
"""
        response = call_openai_api_multi_turn(
            messages=[{"role": "user", "content": weaker_llm_prompt}],
            model_name=self.weaker_llm_model
        )
        return response


    def optimize_library_with_running_weaker_llm(self, SIB: str = ""):
        """
        Optimize the library by running weaker LLM with tools, then having stronger LLM analyze the results.
